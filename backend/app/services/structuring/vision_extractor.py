    ...
    def _extract_text_from_image(
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if attempt >= self.max_retries or not self._should_retry(status_code):
                    logger.exception(
                        "Failed to extract text from image (page %s)", page_number
                    )
                    break

                logger.warning(
                    "Vision API extraction attempt %s/%s failed for page %s (status=%s): %s",
                    attempt,
                    self.max_retries,
                    page_number,
        ...
        if last_error is not None:
            logger.debug("Vision extraction last error detail: %s", last_error)
