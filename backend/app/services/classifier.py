...
class DocumentClassifier:
    ...
    def classify(self, *, filename: str, text_sample: str) -> Optional[ClassificationResult]:
        """Return the best classification using templates with optional LLM assistance."""
        ...
        if not self._llm_enabled or not self._openai_client:
            if not self._llm_enabled:
                logger.debug("LLM分類は無効化されているためテンプレート判定のみを使用します")
            elif not self._openai_client:
                logger.info(
                    "OpenAIクライアントを初期化できなかったためテンプレート判定にフォールバックします"
                )
    ...
    def _classify_with_llm(
        ...
        *,
        ...
        try:
            ...
            logger.warning("Failed to parse OpenAI classification payload: %s", exc)
            return None
