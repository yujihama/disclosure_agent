...
class DocumentClassifier:
    ...
    def classify(self, *, filename: str, text_sample: str) -> Optional[ClassificationResult]:
        """Classify a document using template heuristics with optional LLM refinement."""
        ...
        if not self._llm_enabled or not self._openai_client:
            if not self._llm_enabled:
                logger.debug("LLM分類は無効のためテンプレート判定のみを使用します")
            else:
                logger.warning(
                    "OpenAIクライアントが利用できないためテンプレート判定を返します"
                )
    ...
    def _classify_with_llm(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "OpenAI classification request failed: %s", exc, exc_info=True
            )
            return None
        ...
        try:
            ...
            logger.warning(
                "Failed to parse OpenAI classification payload: %s", exc, exc_info=True
            )
            return None
    ...
    def _build_openai_client(self) -> Any | None:
        ...
        try:
            ...
            logger.warning(
                "Failed to initialise OpenAI client: %s", exc, exc_info=True
            )
            return None
